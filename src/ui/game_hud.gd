...
func _draw_aa_world_axes() -> void:
	...
	for p in match_ctrl.players:
		...
		if p.is_local and not tp:
			continue
		_world_axis(cam, p, p.aa.real_yaw, Color(1.0, 0.22, 0.16, 0.95), "REAL", 1.85)
		_world_axis(cam, p, p.aa.fake_yaw, Color(0.22, 0.50, 1.0, 0.92), "FAKE", 1.45)
		_world_axis(cam, p, p.aa.lby, Color(1.0, 0.86, 0.14, 0.95), "LBY", 1.10)
...
func _world_axis(cam: Camera3D, p: Player, yaw: float, col: Color, label: String, length: float) -> void:
	...
	if cam.is_position_behind(o):
		return
	# AA backwards points at the TP camera; still draw a shortened segment.
	if cam.is_position_behind(tip):
		for _i in 6:
			tip = o.lerp(tip, 0.55)
			if not cam.is_position_behind(tip):
				break
	var a: Vector2 = cam.unproject_position(o)
	var b: Vector2 = cam.unproject_position(tip)
	draw_line(a, b, Color(0, 0, 0, 0.6), 6.0)
	draw_line(a, b, col, 3.5)
	draw_circle(b, 6.0, col)
	_t(b + Vector2(7, -2), label, 12, col, true)
