...
func _draw_menu(sz: Vector2) -> void:
	...
	match _tab:
		...
		3:
			...
			y = _sl(x, y, "View FOV", "visuals/fov", 70, 130)
			_t(Vector2(x, y + 12), "MM $800  pistol Duals/Deagle/R8  then Scout/AWP/Auto", 11, Color(0.6, 0.62, 0.55))
...
func _sl(x: float, y: float, label: String, path: String, lo: float, hi: float) -> float:
	var v := float(Cheat.t(path, lo))
	_t(Vector2(x, y), "%s  %d" % [label, int(v)], 13, Color(0.85, 0.88, 0.9), true)
